...
const prdContentRoot = path.resolve(
    docsSiteRoot,
    "src",
    "content",
    "docs",
...
const planningMappings = [
    ...
    {
        source: path.resolve(sourceDocsRoot, "features", "FLOW_ORACLE.md"),
        dest: path.resolve(docsContentRoot, "flow-oracle.md"),
        title: "Flow Oracle",
    },
    {
        source: path.resolve(
            sourceDocsRoot,
            "features",
            "meeting-detail-branch-ui-atlas.md",
        ),
        dest: path.resolve(
            docsContentRoot,
            "meeting-detail-branch-ui-atlas.md",
        ),
        title: "미팅 상세 Branch UI Atlas",
    },
    {
...
const prdFiles = (await readdir(sourcePrdRoot)).filter((name) =>
    name.endsWith(".md"),
);
...
for (const prdFile of prdFiles) {
    ...
    await writeFile(
        dest,
        withFrontMatter(titleFromFileName(prdFile), sourceBody),
        "utf8",
    );
}
